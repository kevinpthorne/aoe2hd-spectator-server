...
abstract class WsServer implements WsComponentInterface
{
    ...
    protected function _tick()
    {
        ...
        foreach ($this->heldMessages as $key => $hm) {
            $found = false;
            foreach ($this->clients as $currentUser) {
            ...
            foreach ($read as $socket) {
                if ($socket == $this->master) {
                    ...
                    if ($numBytes === false) {
                        ...
                        $user = $this->getClientBySocket($socket);
                        if (!$user->handshake) {
    ...
    {
        $user = new Client(uniqid('u'), $socket);
    ...
    {
        $disconnectedUser = $this->getClientBySocket($socket);

        if ($disconnectedUser !== null) {
            unset($this->clients[$disconnectedUser->id]);
            ...
            if ($triggerClosed) {
                ...
                $this->closed($disconnectedUser, $this);
    ...
    protected function doHandshake($client, $buffer)
    {
        ...
        if (isset($headers['get'])) {
            $client->requestedResource = explode("?", $headers['get'])[0];
            $client->query = $this->extractQuery($headers['get']);
        } else {
        ...
        if (isset($handshakeResponse)) {
            socket_write($client->socket, $handshakeResponse, strlen($handshakeResponse));
    ...
    protected function frame($message, $client, $messageType = 'text', $messageContinues = false)
    {
        switch ($messageType) {
            case 'continuous':
                $b1 = 0;
        ...
        if ($messageContinues) {
            $client->sendingContinuous = true;
        } else {
            !isset($b1) ? $b1 = 128 : $b1 += 128;
            $client->sendingContinuous = false;
        }
            ...
            if (($message = $this->deframe($frame, $user, $headers)) !== FALSE) {
                if ($user->hasSentClose) {
                    ...
                    if ((preg_match('//u', $message)) || ($headers['opcode'] == 2)) {
                        ...
                        $this->process($user, $message, $this);
                    } else {
    ...
    protected function deframe($message, &$client)
    {
    ...
    protected function checkRSVBits($headers, $client)
    { // override this method if you are using an extension where the RSV bits are used.
        if (ord($headers['rsv1']) + ord($headers['rsv2']) + ord($headers['rsv3']) > 0) {
            //$this->disconnect($client); // todo: fail connection
            return true;
        }
        return false;
    }

    protected function extractQuery($requestedResourcePath) {
        $query = array();
        $splitQueryFromPath = explode("?", $requestedResourcePath);
        if(count($splitQueryFromPath) > 1) {
            $queryString = $splitQueryFromPath[1];
